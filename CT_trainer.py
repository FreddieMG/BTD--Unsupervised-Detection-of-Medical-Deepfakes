...
class Dataset(Dataset):
    def __init__(self):
        ...
        df_train['unix_path'] = df_train['unix_path'].apply(lambda x: x.replace('new_dataset/', ''))
        # df_train = df_train[df_train['istumor']==0]
        scanner_list = {'LightSpeed16': 0, 'Sensation 16': 1, 'LightSpeed Pro 16': 2, 'LightSpeed Ultra': 3, 'LightSpeed QX': 4}

        location_list = set()
        for y, x in zip(df_train['y'], df_train['x']):
            try:
                location_list.add((int(y), int(x)))
            except:
                pass
    ...
    def __getitem__(self, idx):
        ...
        file_path = row['unix_path']
        image = np.load(os.path.join(self.path, file_path))
        image = np.clip(image, -700, 2000)
        image = (image+700) / 2700
        y, x = self.location_list[randint(0, len(self.location_list)-1)]
        if 50 - y > 0:
            y += 50-y
        if 50 - x > 0:
            x += 50-x
        ...
        sample = image[y-48: y+48, x-48:x+48].reshape(1, 96, 96) 
